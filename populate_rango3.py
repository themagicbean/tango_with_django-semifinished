...
def populate():
	...
	#from stack, need nested loop to list through list (of dicts) then dicts
    for dicts in cats:
        cat = dicts["category"]
    ...
    for c in Category.objects.all():
        for p in Page.objects.filter(category=c):
            print("- {0} - {1} views: {2} likes: {3}".format(str(c), str(p), int(p.views), int(p.likes)))
            #bracketed numbers are placeholders for later outputs (str c and p)
            #views and likes seem to be carrying from cat to page ... oh well project for later
